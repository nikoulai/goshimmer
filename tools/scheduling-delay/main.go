...
func runBackgroundAnalysis(bgChans *backgroundAnalysisChan) {
	spamWithNodes(nameNodeInfoMap["master"].client, bgChans.shutdown)
...
func spamWithNodes(api *client.GoShimmerAPI, shutdown chan struct{}) {
	...
	for _, seed := range seeds {
		...
		id := identity.ID{}
		idRaw, _ := base58.Decode(base58.Encode(nodeIdentity.ID().Bytes()))
		copy(id[:], idRaw)
		fmt.Println(base58.Encode(nodeIdentity.ID().Bytes()) == base58.Encode(id.Bytes()))
