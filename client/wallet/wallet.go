...
func (wallet *Wallet) SendFunds(stringPayload string, options ...sendoptions.SendFundsOption) (tx *ledgerstate.Transaction, err error) {
	...
	payloadToSend := payload.NewGenericDataPayload([]byte(stringPayload))
	txEssence := ledgerstate.NewTransactionEssence(0, time.Now(), aPledgeID, cPledgeID, inputs, outputs, payloadToSend)
...
func (wallet *Wallet) ConsolidateFunds(options ...consolidateoptions.ConsolidateFundsOption) (txs []*ledgerstate.Transaction, err error) {
	...
	confirmedAvailableBalance, _, err, _ := wallet.AvailableBalance()
	if err != nil {
		return
	}
	...
	for _, consumedOutputs := range consumedOutputsSlice {
		...
		txEssence := ledgerstate.NewTransactionEssence(0, time.Now(), aPledgeID, cPledgeID, inputs, outputs, nil)
		outputsByID := consumedOutputs.OutputsByID()
...
func (wallet *Wallet) CreateNFT(options ...createnftoptions.CreateNFTOption) (tx *ledgerstate.Transaction, nftID *ledgerstate.AliasAddress, err error) { // build options from the parameters
	...
	// create tx essence
	outputs := ledgerstate.NewOutputs(unsortedOutputs...)
	txEssence := ledgerstate.NewTransactionEssence(0, time.Now(), accessPledgeNodeID, consensusPledgeNodeID, inputs, outputs, nil)
...
func (wallet *Wallet) DestroyNFT(options ...destroynftoptions.DestroyNFTOption) (tx *ledgerstate.Transaction, err error) {
	...
	essence := ledgerstate.NewTransactionEssence(0, time.Now(), accessPledgeNodeID, consensusPledgeNodeID,
		ledgerstate.NewInputs(inputs...), ledgerstate.NewOutputs(outputs...), nil)
...
func (wallet Wallet) SweepNFTOwnedFunds(options ...sweepnftownedoptions.SweepNFTOwnedFundsOption) (tx *ledgerstate.Transaction, err error) {
	...
	essence := ledgerstate.NewTransactionEssence(0, time.Now(), accessPledgeNodeID, consensusPledgeNodeID, ledgerstate.NewInputs(unsortedInputs...), ledgerstate.NewOutputs(unsortedOutputs...), nil)
...
func (wallet *Wallet) AvailableBalance(refresh ...bool) (confirmedBalance, pendingBalance map[ledgerstate.Color]uint64, err error, unspentOutputs OutputsByAddressAndOutputID) {
	shouldRefresh := true
	if len(refresh) > 0 {
		shouldRefresh = refresh[0]
	}
	...
	now := time.Now()
	// iterate through the unspent outputs
	outputAddresses := wallet.outputManager.UnspentOutputs(true)

	for addy, outputsOnAddress := range outputAddresses {
		for _, output := range outputsOnAddress {
			// determine target map
			var targetMap map[ledgerstate.Color]uint64
			if output.GradeOfFinalityReached {
	...
	return confirmedBalance, pendingBalance, err, outputAddresses
}
