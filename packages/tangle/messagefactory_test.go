...
func TestMessageFactory_BuildMessage(t *testing.T) {
	...
	t.Run("CheckProperties", func(t *testing.T) {
		...
		msg, err := tangle.MessageFactory.IssuePayload(p, tangle.Options.Identity)
...
func TestMessageFactory_POW(t *testing.T) {
	...
	msg, err := msgFactory.IssuePayload(payload.NewGenericDataPayload([]byte("test")), tangle.Options.Identity)
	require.NoError(t, err)
...
func TestWorkerFunc_PayloadSize(t *testing.T) {
	...
	msg, err := msgFactory.IssuePayload(payload.NewGenericDataPayload(data), testTangle.Options.Identity)
	require.NoError(t, err)
